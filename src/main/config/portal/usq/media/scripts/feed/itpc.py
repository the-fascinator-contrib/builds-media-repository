...
class ItpcData:
    ...
    def checkItunesSupported(self, fileList=[]):
        iTunesSupported = [".m4a", ".mp3", ".mp4"]
        if fileList:
            for filename in fileList:
                if os.path.splitext(filename)[1] in iTunesSupported:
                    return filename
        return ""
    
    def convertDuration(self, duration):
        #assume duration created by fascinator is in 1h1m1s format
        #convert to iTunes supported format h:m:s
        formatted = ""
        if duration.find("h")>0:
            hour, minSec = duration.split("h")
            formatted = "%s:" % hour.strip()
        else:
            minSec = duration
        if minSec.find("m")>0:
            min, sec = minSec.split("m")
            formatted += "%s:" % min.strip()
        else:
            sec = duration
        if sec.find("second(s)"):
            sec = sec[:sec.find("second(s)")]
        elif sec.find("s"):
            sec = sec[:-1]
        formatted += "%s" % sec.strip()
        return formatted
    
    def getString(self, stringList):
        string = ""
