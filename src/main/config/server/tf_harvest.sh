...
echo "The Fascinator - USQ Media Repository - Harvest Client"
...
if running; then
	...
	if [ -f $JSON_FILE ]; then
		...
		$JAVA_HOME/bin/java $JAVA_OPTS -cp $CLASSPATH com.googlecode.fascinator.HarvestClient $JSON_FILE $ARGS > $LOG_FILE 2>&1
